...
def val_kitti_temporal_filtering():
    ...
    load_model(model01, cfg20.proj_home + 'checkpoint/kitti-1-0.1-(0.5,5).pth', cfg2cm.device)
    ...
    with torch.no_grad():
        for cnt_ in range(start, end):
            ...
            with open(cfg20.proj_home + 'results/' + log_folder + '/log.txt', 'a') as writer:
                writer.write(str(cnt_) + ' ' + str(100) + '\t')
            ...
            for cnt in range(len(kitti_dataset20)):
                ...
                predrotmat = eulerAnglesToRotationMatrix([pred_Rot[0] * (3.141592 / 180.0), pred_Rot[1] * (3.141592 / 180.0), pred_Rot[2] * (3.141592 / 180.0)])
                predRTmat = np.zeros((4, 4), dtype=np.float32)
                predRTmat[:3, :3] = predrotmat[:3,:3]
                predRTmat[0, 3] = pred_Trs[0]
                predRTmat[1, 3] = pred_Trs[1]
                predRTmat[2, 3] = pred_Trs[2]
                predRTmat[3, 3] = 1
                
                # error_MAT = predRTmat @ misRTmat
                # error_ROT = rotationMatrixToEulerAngles(error_MAT[:3, :3]) * (180.0 / np.pi)
                # error_TRS = error_MAT[:3, 3]
                # print(cnt, np.abs(error_ROT), np.abs(error_TRS))
